...
public class MultiThreadExample2 {
    ...
    public static void main(String[] args) {
        ...
        // 任意对象都有一个锁和一个等待队列线程
        // synchronized 执行大概过程：
        // 1. 尝试获取对象锁，成功进入第二步，否则加入等待队列，阻塞并等待唤醒
        // 2. 执行相关的代码块
        // 3. 释放锁并从等待队列线程中随机唤醒一个线程并执行。
        // synchronized 可以保证数据获取对象锁的时候从内存中读取，释放锁的时候数据写入内存， 从而保证数据内存的可见性。
        // synchronized: 可重入性，内存可见性，死锁
        //可重入性：获得锁对象之后，调用同用需要锁对象的代码，可以直接调用，可重入性是通过记录锁的持有线程和持有数量来实现的
        //内存可见性：获取锁对象时，从内存中对象最新数据，释放锁时，将数据写入内存中。
        //死锁：避免在持有一个锁的时候去申请另外一个锁。
