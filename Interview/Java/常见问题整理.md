...
23. AbstractQueuedSynchronized（同步器）
        ...
        // parkAndCheckInterrupt使用LookSupport阻塞当前线程，线程唤醒返回时需要检查中断状态
...
24. ReentrantLock
            ...
            protected final boolean tryAcquire(int acquires) {
                ...
                if (c == 0) {
                    ...
                    if (!hasQueuedPredecessors() &&
                        // 如果当前线程是首节点的后继节点则进行CAS获取锁(保证公平性)
        ...
        // 当前线程是否是首节点的后继节点，不是首节点的后继节点返回true,否则返回false
        public final boolean hasQueuedPredecessors() {
            Node t = tail; // Read fields in reverse initialization order
            Node h = head;
            Node s;
            // 同步队列不为空且首节点的后继节点不等于当前线程
            return h != t &&
...
25. ReentrantReadWriteLock
        ...
        概述：
            可重入读写锁，具有ReentrantLock相似的语义。ReentrantReadWriteLock维护着一对相关的锁（读锁和写锁），读锁用于只读操作，写锁用于写入操作。
            如果写锁没有被线程锁持有，那么读锁可以被多个线程锁同时持有。写锁是独占的，一次只能被一个线程所持有。
        重入
            ReentrantReadWriteLock支持ReentrantLock的重入语义：允许线程持有读锁或写锁时，重新获取读锁和写锁(可重入)。当写锁被一个线程锁持有时，其他线程申请
            读锁的线程将会被阻塞直到写锁被释放。
        锁降级
            ReentrantReadWriteLock支持从写锁降级为读锁，实现方式时：先获取写锁，然后获取读锁，最后释放写锁。
            但是ReentrantReadWriteLock不允许读锁升级写锁。
            实例代码（展示了如何利用重入来执行升级缓存后的锁降级）：
             class CachedData {
               Object data;
               ...
               void processCachedData() {
                 ...
                 if (!cacheValid) {
                    ...
                    // 获取写锁
                    rwl.writeLock().lock();
                    // 重新检查缓存状态，因为在获取写锁期间，其他线程可能已经获取到写锁并更改了缓存
                    if (!cacheValid) {
                      // 更新缓存数据
                      data = ...
                      cacheValid = true;
        ...
        锁中断
            ReentrantReadWriteLock支持读锁和写锁获取期间进行中断操作。
        Condition支持
            ReentrantReadWriteLock支持写锁Condition，但是不支持读锁Condition。


        ReentrantReadWriteLock一般典型用法如下：
        class X {
        ...
        final Sync sync;
        // 构造函数
        // 由fair参数决定锁实现策略是公平还是非公平。公平还是非公平区别参考ReentrantLock说明。
        public ReentrantReadWriteLock(boolean fair) {
            sync = fair ? new FairSync() : new NonfairSync();
            readerLock = new ReadLock(this);
            writerLock = new WriteLock(this);
        }
        // AQS实现了读锁和写锁的基本框架，Sync是ReentrantReadWriteLock的内部类，它继承了AQS类并实现了读锁和写锁的具体逻辑。
        // Sync成员变量
        // 因为AQS使用state来管理锁状态和锁数量。这里为了区分读锁和写锁, 将int state拆分成高低16位来存储读锁和写锁。
        // state高16位存储读锁，低16位存储写锁
        // 这里成员变量是为了方便操作位向量
        static final int SHARED_SHIFT   = 16;
        static final int SHARED_UNIT    = (1 << SHARED_SHIFT);
        static final int MAX_COUNT      = (1 << SHARED_SHIFT) - 1;
        static final int EXCLUSIVE_MASK = (1 << SHARED_SHIFT) - 1;
        // 获取读锁数量(因为读锁存储在高16位，所以需要无符号右移16位)
        static int sharedCount(int c)    { return c >>> SHARED_SHIFT; }
        // 获取写锁数量(因为写锁存储在低16位，所以需要与EXCLUSIVE_MASK(高16位全部为0，低16为全部为1)进行按位与取得)。
        ...
        // 当前线程的读锁信息（使用ThreadLocal进行存储）
        private transient ThreadLocalHoldCounter readHolds;
        // 缓存最后一次成功获取读锁信息
        private transient HoldCounter cachedHoldCounter;
        // 缓存第一个成功获取读锁（第一个CAS成功将同步状态修改成1的线程）的线程和读锁的数量
        private transient Thread firstReader;
        private transient int firstReaderHoldCount;
        // 读锁实现分析
        // 获取读锁
        public void lock() {
            // 调用AQS的acquireShared方法（详细流程见AQS分析），acquireShared会回调Sync的tryAcquireShared实现
            sync.acquireShared(1);
        }
        // 获取读锁的具体实现(返回值决定是否可以成功获取锁和后续节点线程是否能够继续获取锁，详细流程参见AQS分析)
        protected final int tryAcquireShared(int unused) {
            // 当前线程
            Thread current = Thread.currentThread();
            // 锁状态
            int c = getState();
            // 如果存在写锁(写锁数量不等于0)且持有写锁的线程不是当前线程则获取锁失败
            if (exclusiveCount(c) != 0 &&
                getExclusiveOwnerThread() != current)
                return -1;
            // 获取读锁数量
            int r = sharedCount(c);
            if (!readerShouldBlock() && // 具体实现由公平锁和非公平锁来决定（FairSync，NonfairSync）
                r < MAX_COUNT && // 读锁数量小于最大数量(只有16位可以存储，所以最大值为：2^16 - 1 = 65535)
                compareAndSetState(c, c + SHARED_UNIT)) { // CAS修改锁状态，高16位加1
                if (r == 0) { // 读锁数量为0
                    // 缓存第一个成功获取读锁的线程和数量
                    firstReader = current;
                    firstReaderHoldCount = 1;
                } else if (firstReader == current) { // 第一次成功获取读锁的线程等于当前线程
                    // firstReaderHoldCount数量加1
                    firstReaderHoldCount++;
                } else {
                    // 如果不是第一个获取读锁线程，使用cachedHoldCounter缓存最后一个成功获取读锁的线程信息
                    HoldCounter rh = cachedHoldCounter;
                    // 如果cachedHoldCounter值为空(当前线程是第二个成功获取的线程)或者最后成功获取读锁的线程ID不等于当前线程ID(cachedHoldCounter不是当前线程信息)
                    if (rh == null || rh.tid != getThreadId(current))
                        // cachedHoldCounter更新成当前线程的HoldCounter
                        cachedHoldCounter = rh = readHolds.get();
                    else if (rh.count == 0) // 如果最后一次成功获取的线程是当前线程并且其锁的数量为0
                        // 更新线程ThreadLocal信息
                        readHolds.set(rh);
                    // HoldCounter线程数量加1
                    rh.count++;
            ...
            // 如果队列策略readerShouldBlock返回true，读锁数量大于最大值或者CAS设置锁状态失败
            return fullTryAcquireShared(current);
        }
        // 完整获取读锁版本，如果队列策略readerShouldBlock返回true，读锁数量大于等于最大值或者CAS设置锁状态失败
        // 可以说tryAcquireShared流程是一次性简单的尝试没有考虑线程重入获取读锁等情况（因为大多数时候都不需要考虑）来加快并发效率
        final int fullTryAcquireShared(Thread current) {
            ...
            for (;;) {
                ...
                } else if (readerShouldBlock()) { // 如果队列策略不允许
                    // Make sure we're not acquiring read lock reentrantly
                    if (firstReader == current) { // 第一次成功获取读锁的线程等于当前线程(可重入获取读锁)
                        // assert firstReaderHoldCount > 0;
                    } else {
                        // 如果不是可重入获取读锁，则更新本地存储线程的信息
                        if (rh == null) {
                            // cachedHoldCounter缓存着最后一个成功获取读锁的线程信息
                            rh = cachedHoldCounter;
                            // 如果cachedHoldCounter值为空或者最后成功获取读锁的线程ID不等于当前线程ID(cachedHoldCounter不是当前线程信息)
                            if (rh == null || rh.tid != getThreadId(current)) {
                                // 使用当前线程的HoldCounter
                                rh = readHolds.get();
                                // 如果当前线程的持有数量等于0，删除HoldCounter使用初始值
                                if (rh.count == 0)
                                    readHolds.remove();
                            }
                        }
                        // 如果当前线程没有持有读锁，则获取读锁失败
                        // 否则当前线程持有读锁则可以去CAS获取锁
                        if (rh.count == 0)
                            return -1;
                    }
                }
                ...
                if (compareAndSetState(c, c + SHARED_UNIT)) {
                    if (sharedCount(c) == 0) { // 读锁数量为0
                        ...
                        if (rh == null)
                            rh = cachedHoldCounter;
                        // 如果rh值为空(当前线程是第二个成功获取的线程)或者最后成功获取读锁的线程ID不等于当前线程ID(cachedHoldCounter不是当前线程信息)
                        if (rh == null || rh.tid != getThreadId(current))
                            // rh更新成当前线程的HoldCounter
                            rh = readHolds.get();
                        else if (rh.count == 0)
                            // 更新线程ThreadLocal信息
                            readHolds.set(rh);
        ...
        // readerShouldBlock允许策略(线程是否尝试获取读锁)，公平锁和非公平锁类有所不同
        // 公平锁实现（FairSync）
        static final class FairSync extends Sync {
        ...
        // hasQueuedPredecessors由AQS实现，当前线程不是首节点的后继节点
        public final boolean hasQueuedPredecessors() {
            Node t = tail; // Read fields in reverse initialization order
            Node h = head;
            Node s;
        ...
        // 由AQS实现，如果同步队列的下一个节点线程是请求互斥锁(写锁)则返回true
        // 这个是为了避免无线程写入操作的饥饿(如果一直保证新线程的读操作，那么等待的写操作线程就会一直阻塞下去)
        ...
        // 总结获取读锁：
            1. 如果存在写锁，获取读锁失败。
            2. 获取策略是否允许readerShouldBlock（公平锁和非公平锁策略有所不同，false允许，true不允许）。
            3. 策略允许获取读锁和读锁数量没有大于最大值，简单尝试获取锁
            4. 策略不允许或者数量大于最大值或者尝试获取锁失败，则完整尝试获取锁(考虑了线程的可重入性，读锁的最大值等情况)
        // 释放读锁
        public void unlock() {
            // 调用AQS的releaseShared方法（详细流程见AQS分析），releaseShared会回调Sync的tryReleaseShared实现
        ...
        protected final boolean tryReleaseShared(int unused) {
            ...
            if (firstReader == current) {
                ...
                // 上次最后一次成功获取锁的线程信息
                HoldCounter rh = cachedHoldCounter;
                // 如果rh为空(说明当前线程是第二个成功获取的线程)或者最后一次成功获取锁的线程ID不等于当前线程ID
                if (rh == null || rh.tid != getThreadId(current))
                    // rh更新为当前线程的ThreadLocal的值
                    rh = readHolds.get();
                // 当前线程持有锁的数量
                int count = rh.count;
                // 当前线程持有锁的数量小于1
                if (count <= 1) {
                    // 线程的ThreadLocal的值恢复使用初始值
                    readHolds.remove();
                    // 异常情况（线程没有获取读锁就去释放读锁）
                    if (count <= 0)
        ...
        // 获取写锁（以独占模式获取）
        public void lock() {
            // 调用AQS的acquire方法（详细流程见AQS分析），acquire会回调Sync的tryAcquire实现
            sync.acquire(1);
        }
        // 尝试获取写锁
        protected final boolean tryAcquire(int acquires) {
        ...
        // writerShouldBlock写锁策略，公平锁和非公平锁实现有所不同
        // 公平锁版本(FairSync)
        static final class FairSync extends Sync {
            private static final long serialVersionUID = -2274990926593161451L;
            final boolean writerShouldBlock() {
                // 如果当前线程不是首节点的后继节点则策略不允许
                return hasQueuedPredecessors();
            }
        }
        // 非公平锁版本(NonfairSync)
        ...
        // 获取写锁步骤：
            1. 如果写锁或者读锁数量大于0且持有锁的线程不是当前线程获取失败
            2. 写锁数量超过最大值抛出异常
            3. 可重入获取写锁或者策略允许CAS尝试获取写锁
        // 释放写锁
        public void unlock() {
            // 调用AQS的release方法（详细流程见AQS分析），release会回调Sync的tryRelease实现
            sync.release(1);
        }
        // 释放写锁具体逻辑
        protected final boolean tryRelease(int releases) {
            // 如果没有持有写锁抛出IllegalMonitorStateException异常
            if (!isHeldExclusively())
                throw new IllegalMonitorStateException();
            // 锁数量减1
...
26. Semaphore
        ...
        计数信号量，维护一个许可集(许可数量)。当许可不可使用时（许可数量等于0），申请许可(acquire)的线程会一直阻塞直到许可可用(许可数量大于0)，成功申请到许可会导致Semaphore的许可数量减1；
        release会释放线程占用的许可并归返给Semaphore(许可数量加1)。Semaphore通常限制访问某些资源的线程数目。
        // 实现分析
        // 成员变量
        // 同步机制具体实现，继承AQS内部实现类
        private final Sync sync;
        ...
        // Semaphore同其他锁实现一样，都是通过内部Sync的AQS实现类来实现Semaphore的语义功能。
        // 这里只分析Semaphore获取许可和释放许可流程
        // 获取许可
        public void acquire() throws InterruptedException {
            // 调用AQS的acquireSharedInterruptibly方法(详细流程见AQS分析acquireShared方法（只是多了中断处理）)，acquireSharedInterruptibly会回调子类的tryAcquireShared实现
            sync.acquireSharedInterruptibly(1);
        }
        tryAcquireShared根据是否是公平策略实现有所不同
        // 公平获取许可实现（FairSync）
        static final class FairSync extends Sync {
            FairSync(int permits) {
                super(permits);
            }
            // 严格按照同步队列的FIFO顺序来处理申请许可的线程
            protected int tryAcquireShared(int acquires) {
                // 自旋防止CAS获取许可数量失败
                for (;;) {
                    // 如果当前线程不是首节点的后继节点（也就是当前线程不是下一个等待获取许可的线程）那么获取许可失败(按照先来先获取原则来保证公平性)
                    if (hasQueuedPredecessors())
                        // 获取许可失败，AQS的doAcquireSharedInterruptibly流程(详细见AQS分析)，doAcquireSharedInterruptibly可能会再次尝试获取许可或者导致线程阻塞
                        return -1;
                    // 获取许可数量（使用AQS的state来存储许可数量，由FairSync构造函数调用AQS来进行初始化）
                    int available = getState();
                    // 剩余的许可数量
                    int remaining = available - acquires;
                    // 如果许可数量小于0或者CAS更改许可数量
                    if (remaining < 0 ||
                        compareAndSetState(available, remaining))
                        // 返回remaining，当小于0进入AQS的doAcquireSharedInterruptibly流程(详细见AQS分析)，doAcquireSharedInterruptibly可能会再次尝试获取许可或者导致线程阻塞
                        return remaining;
                }
            }
        }
        // 公平获取许可的步骤：
            1. 如果同步队列不为空且当前线程不是下一个要获取的节点线程则获取许可失败。
            2. 计算剩余的许可数量。
            3. 许可数量小于0获取许可失败。
            4. CAS成功更新许可数量则获取许可成功。
        // 非公平获取许可实现（NonfairSync）
        protected int tryAcquireShared(int acquires) {
            // 调用Sync的nonfairTryAcquireShared方法
            return nonfairTryAcquireShared(acquires);
        }
        // nonfairTryAcquireShared如果许可数量可用会直接尝试去获取许可而不管是否由等待的线程(不公平)
        final int nonfairTryAcquireShared(int acquires) {
            // 自旋防止CAS获取许可数量失败
            for (;;) {
                // 获取许可数量（使用AQS的state来存储许可数量，由NonfairSync构造函数调用AQS来进行初始化）
                int available = getState();
                // 剩余的许可数量
                int remaining = available - acquires;
                // 如果许可数量小于0或者CAS更改许可数量
        ...
        // 非公平获取许可的步骤：
            1. 计算剩余的许可数量。
            2. 许可数量小于0获取许可失败。
            3. CAS成功更新许可数量则获取许可成功。
        // 释放许可
        public void release() {
            // 调用AQS的releaseShared方法(详细流程见AQS分析的releaseShared方法)，releaseShared会回调子类的tryReleaseShared实现
            sync.releaseShared(1);
        }
        // 不管公平还是非公平实现都会使用Sync的tryReleaseShared来释放许可
        ...
        // 释放许可的步骤：
            ...
            2. 许可数量溢出，抛出异常
            2. CAS更改许可数量
        // 通过tryReleaseShared流程分析，释放许可并没有考虑初始许可的大小，也就是说释放许可增加的许可数量可能会超过初始许可数量
        // 这个大家可以通过代码进行实际验证下。


27. CountDownLatch

         同步辅助类，初始化一个计数值，当这个计数不等于0之前，await方法会使线程一直阻塞，countDown会将计数减1，当计数等于0时，会释放
         所有的阻塞等待线程，后续所有线程调用await方法都会立即返回。计数是无法被重置的。
         CountDownLatch有很多应用场景：
            1. 用计数初始化1的CountDownLatch用作一个简单的开/关锁存器（入口），所有调用await的线程将入口处一直阻塞等待，直到线程调用
            countDown打开入口。
            2. 用计数初始化N的CountDownLath可以使一个线程再N个线程完成某项操作之前一直等待或者某项操作完成N次之前一直等待。
            3. 用计数初始化N的CountDownLath可以使一个问题拆分成N个子步骤，N个步骤在不同的线程中进行计算，主线程中await等待子步骤完成后计算问题的结果。
         // 实现分析
         // 成员变量
         // 线程间同步的具体实现类，内部继承AQS实现(AQS流程详细见AQS分析)
         private final Sync sync;
         // 构造函数，初始化计数
         public CountDownLatch(int count) {
             // 非法计数值
             if (count < 0) throw new IllegalArgumentException("count < 0");
             // 初始Sync（调用AQS初始化计数为count）
             this.sync = new Sync(count);
         }
         // await
        public void await() throws InterruptedException {
            // 调用AQS的acquireSharedInterruptibly方法(详细流程见AQS分析acquireShared方法（只是多了中断处理）)，acquireSharedInterruptibly会回调Sync的tryAcquireShared实现
            sync.acquireSharedInterruptibly(1);
        }
        // tryAcquireShared的返回值决定了线程是否需要阻塞等待，true不需要，false需要阻塞等待
        protected int tryAcquireShared(int acquires) {
            // state的值由构造Sync时调用AQS进行初始化，当state值不为0阻塞等待
            return (getState() == 0) ? 1 : -1;
        }
        // countDown
        public void countDown() {
            // 调用AQS的releaseShared方法(详细流程见AQS分析releaseShared方法)，releaseShared会回调Sync的tryReleaseShared实现
            sync.releaseShared(1);
        }
        // tryReleaseShared会将计数减1，当计数等于0时会释放所有等待的节点线程(利用AQS的共享模式实现，详细分析见AQS分析文章)
        // tryReleaseShared的返回值决定了是否需要唤醒的阻塞的节点线程,true需要唤醒，false不需要
        protected boolean tryReleaseShared(int releases) {
            ...
            for (;;) {
                ...
                // 计数值减1
                int nextc = c - 1;
                // CAS更改计数值(CAS详细见原子操作实现原理分析文章)
                if (compareAndSetState(c, nextc))
                    // 计数值是否等于0来决定是否唤醒等待的节点线程
                    return nextc == 0;
            }
        }
...
28. CyclicBarrier
        ...
        同步辅助类，允许一组线程互相阻塞等待直到所有线程到达公共屏障点(barrier)，当所有线程到达公共屏障点后，阻塞等待的
        线程才会继续执行。屏障点(barrier)在释放阻塞等待线程后可以重用，因此叫做可循环的屏障(Cyclic barrier)。
        // 实现分析
        // 成员变量
        // 可重入互斥锁保证一次只有一个线程进入
        private final ReentrantLock lock = new ReentrantLock();
        // Condition(Condition分析见AQS分析)用来阻塞线程和到达屏障点后唤醒所有线程
        private final Condition trip = lock.newCondition();
        // 参加的线程数量
        private final int parties;
        // 屏障操作，是一个Runnable，由构造函数传入，用来执行一些自定义逻辑。
        // 当所有线程到达屏障点且释放所有阻塞等待线程之前调用。
        private final Runnable barrierCommand;
        // 用来记录CyclicBarrier是否是破坏状态(await中途发送了中断，超时，reset或者barrierCommand执行期间发生了异常会设置barrier破坏状态)
        // 每次使用CyclicBarrier都会生成一个新的实例。
        private Generation generation = new Generation();
        // 仍在等待的线程数量，初始时等于parties，wait会将count减1，当count等于0或者CyclicBarrier被破坏时
        // count会被重置成parties
        private int count;
        // await
        // 每次调用await会导致count数量减一，当count等于0时会唤醒所有阻塞等待的线程
        ...
        private int dowait(boolean timed, long nanos)
            ...
            lock.lock();
            try {
                // Generation实例用来判读barrier是否被破坏或者是否生成新的实例(reset或者到达屏障点后会重新生成新的实例)
                final Generation g = generation;
                // barrier是否被破坏(await中途发送了中断，超时，reset或者barrierCommand执行期间发生了异常会设置barrier破坏状态)
                if (g.broken)
                    // 如果barrier被破坏，抛出BrokenBarrierException异常
                    throw new BrokenBarrierException();
                // 进入方法时线程已经被中断，设置barrier破坏状态，并抛出InterruptedException异常
                if (Thread.interrupted()) {
                    // 设置barrier破坏状态
                    breakBarrier();
                ...
                // count数量减1,同时index记录当前线程唤醒返回时线程的索引下标
                // 第一个到达屏障点的线程(第一个调用await的线程)index为parties-1
                // 最后一个到达屏障点的线程(最后一个调用await的线程)index为0
                int index = --count;
                // index等于0表示所有的线程都达到了屏障点
                if (index == 0) {  // tripped
                    // 标志barrier command运行期间是否有异常
                    boolean ranAction = false;
                    try {
                        // barrier command
                        final Runnable command = barrierCommand;
                        // 如果有barrier command，先执行barrier command
                        if (command != null)
                            command.run();
                        // 标志无异常
                        ranAction = true;
                        // 唤醒所有等待线程，重置count并生成新的Generation实例
                        nextGeneration();
                        // 返回到达屏障点的线程索引
                        return 0;
                    } finally {
                        // 如果barrier command运行期间发生了异常（ranAction为false）设置barrier破坏状态
                        if (!ranAction)
                            breakBarrier();
                    }
                }

                // 循环(防止虚假唤醒)直到所有线程到达barrier或者barrier被破坏
                for (;;) {
                    try {
                        // 不是超时await
                        if (!timed)
                            // 使用Condition的await阻塞线程
                            trip.await();
                        else if (nanos > 0L) // 超时await且超时时间大于0
                            // 使用Condition的awaitNanos阻塞线程
                            nanos = trip.awaitNanos(nanos);
                    } catch (InterruptedException ie) {
                        // 如果线程阻塞等待期间发生了中断
                        // 如果中断发生在signalAll之前且barrier不是被破坏状态
                        if (g == generation && ! g.broken) {
                            ...
                            // 如果中断发生在signalAll之后且barrier关联了新的Generation实例，
                            // 那么设置线程中断标志
                    ...
                    if (g.broken)
                        ...
                        throw new BrokenBarrierException();
                    // barrier关联了新的Generation实例，说明所有线程到达屏障点且被正常释放
                    if (g != generation)
                        // 返回线程索引
                        return index;
                    // 线程awaitNanos发生了超时或者await传入小于0的非法值
                    if (timed && nanos <= 0L) {
                        // 设置barrier破坏状态
                        breakBarrier();
        ...
        // nextGeneration 唤醒所有等待线程，重置count并生成新的Generation实例
        ...
        // barrier每次循环关联新的Generation和重置count达到barrier的循环使用
        // breakBarrier设置barrier破坏状态，重置count并唤醒所有线程
        private void breakBarrier() {
            generation.broken = true;
            count = parties;
            trip.signalAll();
        }
        // breakBarrier设置破坏状态会唤醒所有线程，唤醒的线程会检查barrier是破坏状态从而抛出BrokenBarrierException
        // 也就是说只要阻塞等待的一个线程被破坏(超时，reset或者barrierCommand执行期间发生了异常)，那么所有等待的阻塞线程后续都会抛出BrokenBarrierException异常

29. 同步容器和并发容器总结

        https://juejin.im/post/5d8e38d0f265da5b8072d140
        https://blog.csdn.net/weixin_41884118/article/details/88054094
        https://zhuanlan.zhihu.com/p/49424830
        同步容器： 使用synchronized来实现同步的容器。
            1. Vector
            2. Stack
            3. HashTable
            4. Collections.synchronized生成的同步容器
            同步容器每次只有一个线程能够访问同步方法，其他线程都会等待阻塞，性能比较低下。
            同步容器并不完全保证线程的安全，当多个线程同时对同步容器进行操作时，需要额外的外部同步。
        并发容器：为了解决同步容器的问题和提高线程并发性，JDK1.5引入了并发容器。并发容器采用了多种优化方式来提高
        并发容器的执行效率，核心技术有：锁、CAS(无锁)，COW(读写分离)、分段锁等。
            1. ConcurrentHashMap(HashMap): JDK7 Segment(分段锁), JDK8 CAS
            2. CopyOnWriteArrayList(ArrayList):读操作不加锁，写操作加锁进行拷贝成新的集合，在新的集合中进行操作，然后将新的集合赋值给旧的引用。
            通过volatile来保证操作的可见性。
            3. CopyOnWriteArraySet(HashSet):基于CopyOnWriteArrayList实现，唯一不同是在add时调用的是CopyOnWriteArrayList的addIfAbsent方法，遍历当前Object数组，
            如Object数组中已有了当前原始，则直接返回，如果没有则放入Object数组的尾部，并返回。
            4. ConcurrentSkipListMap(TreeMap)：使用Skip list（跳表）来代替红黑树，因为红黑树在插入或删除节点的时候需要调整，导致需要控制的粒度较大。
            Skip list是一种可以代替平衡树的数据结构，默认是按照Key值升序的，
            5. ConcurrentSkipListSet(TreeSet): 内部使用ConcurrentSkipListMap进行实现。
            6. ConcurrentLinkedQueue(Queue): 基于链表实现的FIFO队列(LinkedList的并发版本)。
            7. LinkedBlockingQueue、ArrayBlockingQueue、PriorityBlockingQueue(BlockingQueue): BlockingQueue拓展了Queue,增加了可阻塞的插入和获取等操作。
               同步ReentrantLock实现线程安全，同步Condition实现阻塞和唤醒。
               LinkedBlockingQueue: 基于链表实现的可阻塞的FIFO队列。
               ArrayBlockingQueue:基于数组实现的可阻塞的FIFO队列。
               PriorityBlockingQueue:按优先级排列的队列。

30. ConcurrentHashMap（JDK1.7）

        https://www.jianshu.com/p/e694f1e868ec
        https://crossoverjie.top/2018/07/23/java-senior/ConcurrentHashMap/
        https://blog.csdn.net/crazy1235/article/details/76795383
        https://www.jianshu.com/p/d767f04102b7
        https://juejin.im/entry/5884f1a7128fe1006c3b6aac
        ConcurrentHashMap使用Segment数组+HashEntry数组链表的方式来实现并发访问，每个Segment关联一把锁，当多线程访问容器例不同Segment的数据时，线程间不会存在竞争关系。
        // 实现分析
        // 成员变量
         // 默认的hash表大小
        ...
        // Segment索引的掩码值，用来定位Segment索引下标
        final int segmentMask;
         // Segment索引的偏移量
        final int segmentShift;
         // Segment数组，每个Segment关联一个hash表
        ...
        public ConcurrentHashMap(int initialCapacity,
            ...
            // Segment数组大小
            int ssize = 1;
            // 计算Segment数组大小（大于等于concurrencyLevel的2的整数次幂）
            ...
            // 计算Segment数组每个Segment关联的hash表的大小（c）
            int c = initialCapacity / ssize;
            if (c * ssize < initialCapacity)
                ++c;
            // 最小hash表的大小（cap）
            int cap = MIN_SEGMENT_TABLE_CAPACITY;
            // 如果cap小于c，进行左移使得cap大于等于c的2的整数次幂的数
            while (cap < c)
                cap <<= 1;
            // create segments and segments[0]
            // 创建Segment0和Segment数组大小
            Segment<K,V> s0 =
        ...
        // Segment继承ReentrantLock，使得Segment具有多线程并发访问的功能
        static final class Segment<K,V> extends ReentrantLock implements Serializable {
                 // tryLock最大重试次数
                static final int MAX_SCAN_RETRIES =
                    Runtime.getRuntime().availableProcessors() > 1 ? 64 : 1;
                  // hash表,每个Segment关联一个HashEntry数组
                ...
                final V put(K key, int hash, V value, boolean onlyIfAbsent) {
                    ...
                    try {
                        ...
                        // 遍历HashEntry链表（HashEntry是一个单链表结构）
                        for (HashEntry<K,V> e = first;;) {
                            ...
                            else { // 如果HashEntry对应下标不存在元素
                                if (node != null)
                                    // 如果node不为null，说明第一次tryLock不成功，其他线程修改了HashEntry数组，我们需要更新HashEntry的位置
                                    node.setNext(first);
                                else
                                    // 新建HashEntry
                ...
                // 尝试进行加锁插入(会尝试64次)
                 private HashEntry<K,V> scanAndLockForPut(K key, int hash, V value) {
                     ...
                     while (!tryLock()) {
                         ...
                         // 尝试次数超过MAX_SCAN_RETRIES挂起线程
                         else if (++retries > MAX_SCAN_RETRIES) {
                             lock();
                             break;
                         }
                         // HashEntry元素已经被更改,重新遍历
                         else if ((retries & 1) == 0 &&
        ...
        public V put(K key, V value) {
            ...
            // 计算key的hash值
            int hash = hash(key);
            // 通过Segment偏移量和掩码值定位Segment下标
            int j = (hash >>> segmentShift) & segmentMask;
            // Segment是否创建(构造初始化时只会创建Segment0的值)
        ...
        // put一般步骤：
            1. 计算key的hash定位Segment表的位置。
            2. 如果Segment不存在ensureSegment进行创建。
            3. 调用Segment的put进行插入
                3-1： tryLock加锁或者加锁失败进行scanAndLockForPut进行循环尝试加锁
                3-2： 根据hash值计算出HashEntry所在的元素链表
                3-3： 遍历HashEntry链表，如果链表不存在新建结点插入，否则替换旧值返回新值
        // get
        // 通过key查找value值
        public V get(Object key) {
            ...
            // key的hash值
            int h = hash(key);
            // 根据hash值计算Segment的内存偏移值
            long u = (((h >>> segmentShift) & segmentMask) << SSHIFT) + SBASE;
            // 获取Segment和Segment的HashEntry
            if ((s = (Segment<K,V>)UNSAFE.getObjectVolatile(segments, u)) != null &&
                (tab = s.table) != null) {
                // hash定位Segment的HashEntry数组的HashEntry元素并遍历HashEntry链表
        ...
        // get一般步骤：
            1. 计算key的hash值
            2. 根据hash值获取Segment和Segment的HashEntry数组
            3. 根据hash值定位HashEntry数组对应的HashEntry元素
            4. 遍历HashEntry找到相关的key对应的value值
        // size
        // 计算HashMap容量大小
        public int size() {
            ...
            try {
                for (;;) {
                    // 当计算次数大于2，也就是sum和last不一致，认定两次计算size之间map的结构发生了修改，对每个Segment进行加锁重新计算
                    ...
                    // 如果两次sum计算结果一致则认定size是最终容量大小退出循环
                    if (sum == last)
                        break;
                    // 记录上一次的sum计算结果
                    last = sum;
        ...
        // 计算size一般步骤：
            1. 两次无锁计算Map的所有Segment的count，modCount之和。
            2. 如果两次无锁计算的modCount之和相等则所有的Segment的count之和为Map的容量大小
            3. 如果两次无锁计算的modCount之和不相等，则第三次计算时对每个Segment进行加锁重新计算所有Segment的count，modCount之和
            4. 返回所有的Segment的count之和
        // rehash
        // 当Segment的HashEntry容量超过阈值时，会进行HashEntry双倍扩容，并把旧的HashEntry数组数据复制到新的HashEntry数组中
        @SuppressWarnings("unchecked")
        private void rehash(HashEntry<K,V> node) {
            ...
            for (int i = 0; i < oldCapacity ; i++) {
                ...
                // 元素不为null(存在HashEntry链表)
                if (e != null) {
                    // 链表下一个结点
                    HashEntry<K,V> next = e.next;
                    // 根据元素的hash值计算元素在新的HashEntry数组中的下标
                    ...
                    if (next == null)   //  Single node on list
                        ...
                        for (HashEntry<K,V> last = next;
                             last != null;
                             last = last.next) {
                            // 根据节点的hash值计算在新数组中的下标
                            int k = last.hash & sizeMask;
                            // 节点不相等，更新lastRun和lastIdx
                            if (k != lastIdx) {
                                lastIdx = k;
                                lastRun = last;
                            }
                        }
                        // 设置lastRun及其之后的节点到新数组中
                        // 上诉for循环就是查找出到链表尾在新数组中具有相同下标的连续节点一次性复制到新数组中
        ...
        // rehash一般步骤：
            1. 新建一个两倍旧数组大小的新数组。
            2. 遍历旧数组, 得到每个HashEntry链表
            3. 遍历HashEntry链表，得到每个元素，根据元素的hash值计算出在新数组中的下标进行拷贝，
                因为旧的HashEntry链表元素的hash计算的下标不一定都在新数组的同一个下标中，这里拷贝的方式如下：
                    1. 将链表尾到lastRun在新数组具有相同下标的节点一下次拷贝到新数组中。
                    2. 将链表头到lastRun节点的元素插入到新数组的中(头插法)。

31. CopyOnWriteArrayList

        ArrayList线程安全的一个变体，所有的读操作不加锁直接进行，所有的写操作加锁并通过复制新数组来实现的，是一种读写分离的并发策略。
        牺牲写操作的效率来提供读操作的高效，适用于读操作大大超出写操作的场景。
        // 实现分析
        // 成员变量
        // 锁的实例变量
        final transient Object lock = new Object();
        // 存储数据的数组，使用volatile来保证内存可见性，使得每次读操作都是上次更新的最新值
        private transient volatile Object[] elements;
        // 构造函数
        public CopyOnWriteArrayList(Collection<? extends E> c) {
            Object[] elements;
            // 如果是CopyOnWriteArrayList，直接getArray获取数据数组
            if (c.getClass() == CopyOnWriteArrayList.class)
                ...
                // 如果集合数组的class本身是Object[]直接复制，否则使用Arrays.copyOf创建新的数组进行复制
        ...
        public E remove(int index) {
            ...
            synchronized (lock) {
                ...
                // 删除的元素是最后一个元素
                if (numMoved == 0)
                    // 直接创建新数组拷贝0到len-1的数据到新数组并更新数组
                    setArray(Arrays.copyOf(elements, len - 1));
                else {
                    // 创建len-1长度的新数组
                    Object[] newElements = new Object[len - 1];
                    // 将旧数组0到index的数据拷贝到新数组0到index-1位置
                    System.arraycopy(elements, 0, newElements, 0, index);
                    // 将旧数组index+1到len的数据拷贝到新数组index到len-1位置
...
32. CopyOnWriteArraySet

        内部采用CopyOnWriteArrayList实例来操作Set接口，为了达到Set没有重复的效果，add方法内部调用CopyOnWriteArraySet的addIfAbsent方法来排除重复元素
        // 实现分析
        // 成员变量
        // 内部使用CopyOnWriteArrayList实例
        private final CopyOnWriteArrayList<E> al;
        // 构造函数
        public CopyOnWriteArraySet(Collection<? extends E> c) {
            // 如果集合是CopyOnWriteArraySet，则新创建一个CopyOnWriteArrayList并它其数组数据复制过去
            if (c.getClass() == CopyOnWriteArraySet.class) {
                ...
                // 否则新建一个CopyOnWriteArrayList，调用addAllAbsent添加集合元素
        ...
        // add
        public boolean add(E e) {
            // 调用CopyOnWriteArrayList的addIfAbsent
            return al.addIfAbsent(e);
        }
        // addIfAbsent
        public boolean addIfAbsent(E e) {
            Object[] snapshot = getArray();
            // 调用indexOf方法判断CopyOnWriteArrayList是否存在元素，否则调用addIfAbsent
            return indexOf(e, snapshot, 0, snapshot.length) >= 0 ? false :
                addIfAbsent(e, snapshot);
        }
        // 循环遍历数组，如果数组存在此元素返回其数组下标，否则返回-1
        ...
        private boolean addIfAbsent(E e, Object[] snapshot) {
            ...
            synchronized (lock) {
                ...
                // 如果当前数组和上次获取的数组不一致，说明其他线程已经更改了数组
                if (snapshot != current) {
        ...
        public boolean remove(Object o) {
            Object[] snapshot = getArray();
            // 如果元素在数组中不存在返回false否则调用remove(Object o, Object[] snapshot, int index)删除元素
        ...
        private boolean remove(Object o, Object[] snapshot, int index) {
            ...
            synchronized (lock) {
                ...
                // 新建len-1的新数组
                Object[] newElements = new Object[len - 1];
                // 拷贝0到index-1的数组数据到新数组中
                System.arraycopy(current, 0, newElements, 0, index);
                // 拷贝index+1到len的数组数据到新数组中
...
33. Skip list(跳表)
        ...
        Skip List是一种随机化的数据结构，基于并联的链表，实现简单，插入、删除、查找的复杂度均为O(logN)（大多数情况下）。
        Skip List通过维护一个多层次的链表，且每一层链表中的元素是钱一层链表元素的子集。一开始，算法在最稀疏的层次进行搜索，直至需要查找的元素
        在该层两个相邻的元素中间。这时，算法将跳转到下一个层次，重复刚才的搜索，直到找到需要查找的元素为止。跳过的元素的方法可以是随机性选择或确定性选择，前者更为常见。
        跳表具有如下性质：
            1. 由很多层结构组成。
            2. 每一层都是一个有序的链表。
            3. 最底层的链表包含所有元素。
        ...
        遍历：
            从上到下由左到右搜索要查找的节点元素
        插入：
            1. 确定该元素要占据的层数K（采用丢硬币的方式，完全随机）。
            2. 在Level 1到Level K各个层的链表都插入元素。
            3. 如果K 大于链表的层数，则要添加新的层。
        删除：
            在各个层中找到包含x的节点，使用标准的delete from list方法删除该节点。

34. ConcurrentSkipListMap

        https://www.cnblogs.com/java-zzl/p/9767255.html
        https://blog.csdn.net/qq_35326718/article/details/78870658
        https://www.jianshu.com/p/7fce19e8fe52
        https://blog.csdn.net/sunxianghuang/article/details/52221913
        ConcurrentSkipListMap提供了一种线程安全的并发访问的排序映射表。内部基于Skip List(跳表)数据结构进行实现，理论上
        能够O(log(n))时间内完成查找、插入、删除操作。
        // 实现分析
        // 节点结构
        // 键值映射对数据节点， 有序的单链表
        ...
        static class Index<K,V> {
            ...
            final Index<K,V> down; // 下一级别索引节点
            volatile Index<K,V> right; // 当前级别索引的后继索引节点
            ......
        }
        // 索引头节点
        ...
        // 成员变量
        // 标记基本级别索引头节点的特殊值（当索引级别等于1， 其索引头节点的value等于BASE_HEADER）
        static final Object BASE_HEADER = new Object();
        // 索引头节点
        private transient volatile HeadIndex<K,V> head;
        // 外部比较器，来维持链表的有序性，如果构造函数中设置了comparator则使用comparator,否则使用Key的Comparable接口
        ...
        // 调用doPut插入，如果Map中存在key，替换value返回旧的value,否则key,value插入到Map返回null
        private V doPut(K key, V value, boolean onlyIfAbsent) {
            ...
            // S0: 查找key要插入的位置，构建Node节点插入数据节点链表
            // 自旋(CAS操作可能失败)
            outer: for (;;) {
                // findPredecessor严格查找小于key的基本索引级别的Node节点，如果不存在返回基本级别索引头节点的Node节点
                // b：要插入key节点的前驱节点
                // n: 要插入key节点的后继节点
                for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                    ...
                    if (n != null) {
                        ...
                        if (n != b.next)               // inconsistent read
                            break;
                        // 后继节点已被标记删除(value等于null)，删除后继节点并重试
                        if ((v = n.value) == null) {   // n is deleted
                            n.helpDelete(b, f);
                            break;
                        }
                        // 前驱节点被标记删除或者后继节点被标记，重试
                        if (b.value == null || v == n) // b is deleted
                            break;
                        // key节点大于后继节点，则b有误，继续向后查找
            ...
            // 将随机数与1000 0000 0000 0000 0000 0000 0000 0001进行异或与操作
            // 如果等于0(测试高位和低位)
            if ((rnd & 0x80000001) == 0) { // test highest and lowest bits
                ...
                while (((rnd >>>= 1) & 1) != 0)
                    ++level;
                // 如果概率计算的level在当前调班level范围内，构建一个从level到1的纵列索引节点链表
                Index<K,V> idx = null;
                ...
                // S2: 将构建的纵列索引idx列表插入到各层索引链表的合适位置
                splice: for (int insertionLevel = level;;) {
                    int j = h.level;
                    // 从左到右，从上到下遍历索引节点
                    for (Index<K,V> q = h, r = q.right, t = idx;;) {
                        // q已被删除或者要插入的纵向索引t为null则直接退出
                        if (q == null || t == null)
                            break splice;
                        // 后继节点不等于null
                        if (r != null) {
                        ...
                        // 在当前索引层找到要插入的位置(如果j != insertionLevel说明要插入的层数小于最大的level)
                        if (j == insertionLevel) {
                            // 将索引t节点插入到q和r索引节点之间，如果插入失败重试
                            if (!q.link(r, t))
                                break; // restart
                            // t已被标记删除则退出进入外层循环重试
        ...
        // put的一般步骤如下:
            1. 查找key要插入的位置，构建Node节点插入数据节点链表。
            2. 随机向上构建索引层链表idx。
            3. 将构建的索引层链表idx插入到各层索引链表的合适位置。
        // get
        // 返回key关联的value,如果不存在key所对应的键值对则返回null
        public V get(Object key) {
            return doGet(key);
        }
        private V doGet(Object key) {
            ...
            outer: for (;;) {
                ...
                for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                    ...
                    if (b.value == null || v == n)  // b is deleted
                        break;
                    // 找到Map的对应的key，返回key对应的value
                    if ((c = cpr(cmp, key, n.key)) == 0) {
                        @SuppressWarnings("unchecked") V vv = (V)v;
                        return vv;
                    }
                    // 如果key小于后继节点的key，则不存在这样的键值对，退出循环
                    if (c < 0)
                        break outer;
                    // 更新前驱节点和后继节点
                    b = n;
        ...
        // remove
        // 根据key删除对应的映射对，返回删除key对应的value或者null(如果不存在key对应的映射对)
        public V remove(Object key) {
        ...
        final V doRemove(Object key, Object value) {
            ...
            outer: for (;;) {
                ...
                for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                    ...
                    if ((c = cpr(cmp, key, n.key)) < 0)
                        break outer;
                    // 如果key大于后继节点的key，更新b,n继续向后遍历
                    if (c > 0) {
                        b = n;
                        n = f;
                        continue;
                    }
                    // 如果传入的value跟key对应的value不匹配，退出循环
                    ...
                    if (!n.appendMarker(f) || !b.casNext(n, f))
                        // 如果CAS失败调用 findNode 删除所有 value 为 null 的结点
                        findNode(key);                  // retry via findNode
                    else {
                        // 清除index
                        findPredecessor(key, cmp);      // clean index
                        // 如果删除节点后导致某一索引层没有其他节点了，并适情况删除该层索引
                        if (head.right == null)
                            tryReduceLevel();
                    }
                    @SuppressWarnings("unchecked") V vv = (V)v;
